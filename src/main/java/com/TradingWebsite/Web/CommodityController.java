...
//            }
     ...
     * @return
     */
    @PostMapping("/updateCommodityInfoById")
    public JSONObject updateCommodityInfoById(HttpServletRequest request) {
        JSONUtil jsonUtil = new JSONUtil();
        User user = (User) request.getSession().getAttribute("user");
        if (user != null) {
            long id = Long.parseLong(request.getParameter("id"));
            if (commodityService.updateCommodityInfoById(id) != true) {
                return jsonUtil.fail("下架失败");
            }
            return jsonUtil.success("下架成功");
        }
        return jsonUtil.fail("暂无权限,请登录或激活账户");
     ...
     * @return
     */
    @PostMapping("/updateWaresInfoById")
    public JSONObject updateWaresInfoById(HttpServletRequest request, Commodity commodity) {
        JSONUtil jsonUtil = new JSONUtil();
        Manager manager = (Manager) request.getSession().getAttribute("admin");
        if (manager != null) {
            try {
     ...
     * @return PageInfo<Commodity>
     */
    @PostMapping("/findwaresbyname_Admin")
    public PageInfo<Commodity> findWaresByNameByAdmin(HttpServletRequest request, @RequestParam("name") String name, @RequestParam("page") String page) {
        String wares = request.getParameter("name");
        int page1 = Integer.parseInt(request.getParameter("page"));
        PageHelper.startPage(page1, 10);//开启分页
        PageInfo<Commodity> pageInfo = new PageInfo<>(commodityService.findByNameOfCommodity_Admin(wares));
        return pageInfo;
    }
